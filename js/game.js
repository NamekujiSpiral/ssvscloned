(() => {
  ...
  class Player {
    constructor(y, def) {
      this.size = 180; this.x = (VIRTUAL_WIDTH - this.size)/2; this.y = y;
      this.color = def.color; this.skills = def.skills;
      this.alive = true; this.direction = 0; this.speed = 3;
      this.cost = 0; this.maxCost = 10; this.costRate = 0.5;
  ...
  class Bullet {
    constructor(x,y,vx,vy,owner,skill){
      ...
      this.owner=owner; this.size=skill.size; this.behavior=skill.behavior; this.passed=false;
    }
    update(dt) {
      if(gameOver) return;
      if(this.behavior==='curveLeft' && !this.passed) {
        if((this.vy<0&&this.y<VIRTUAL_HEIGHT/2)||(this.vy>0&&this.y>VIRTUAL_HEIGHT/2)){
          this.passed=true; this.vx=-6;
        }
      }
  ...
  function resize(){
    const w=innerWidth, h=innerHeight, r=VIRTUAL_WIDTH/VIRTUAL_HEIGHT;
    let dw,dh;
  ...
  function fire(pl,idx){
    const sk=pl.skills[idx];
    if(pl.cost<sk.cost||pl.cooldowns[idx]>0||gameOver) return;
    pl.cost-=sk.cost; pl.cooldowns[idx]=COOLDOWN_DURATION;
    const bx=pl.x+pl.size/2, by=pl.y+(pl===p1?-pl.size/2:pl.size+pl.size/2);
    const vy=pl===p1?-sk.speed:sk.speed;
    bullets.push(new Bullet(bx,by,0,vy,pl,sk));
  }

  function onHit(t){gameOver=true;breakTarget=t;}

  function checkHit(b){
    const t=b.owner===p1?p2:p1; if(!t.alive||gameOver) return false;
    const h=b.size/2;
  ...
  canvas.addEventListener('pointerdown',e=>{
    ...
    if(y<virtualButtonHeight){pressedButton={pl:p2,idx};return;}
    if(y>VIRTUAL_HEIGHT-virtualButtonHeight){pressedButton={pl:p1,idx};return;}
  ...
  function loop(now){
    ...
    for(let i=0;i<3;i++){
      ...
      const pl2=p2, pl1=p1;
      const us2=pl2.cost>=pl2.skills[i].cost&&pl2.cooldowns[i]<=0;
      const us1=pl1.cost>=pl1.skills[i].cost&&pl1.cooldowns[i]<=0;
      ctx.fillStyle=us2?pl2.color:'rgba(200,200,200,0.9)';ctx.fillRect(x0,y0p,bwPx-2,bhPx-2);
      ctx.fillStyle=us1?pl1.color:'rgba(200,200,200,0.9)';ctx.fillRect(x0,y0m,bwPx-2,bhPx-2);
      if(pressedButton&&pressedButton.pl===pl2&&pressedButton.idx===i)ctx.fillStyle='rgba(0,0,0,0.3)',ctx.fillRect(x0,y0p,bwPx-2,bhPx-2);
      if(pressedButton&&pressedButton.pl===pl1&&pressedButton.idx===i)ctx.fillStyle='rgba(0,0,0,0.3)',ctx.fillRect(x0,y0m,bwPx-2,bhPx-2);
      // フォント設定
      ctx.fillStyle='#fff';
      ctx.font=`${Math.floor(bhPx*0.25)}px Gonta`; // 技名小さく
      ctx.fillText(pl2.skills[i].name, x0+bwPx/2, bhPx*0.35);
      ctx.font=`${Math.floor(bhPx*0.3)}px Gonta`; // コスト
      ctx.fillText(pl2.skills[i].cost, x0+bwPx/2, bhPx*0.75);
      ctx.font=`${Math.floor(bhPx*0.25)}px Gonta`;
      ctx.fillText(pl1.skills[i].name, x0+bwPx/2, y0m+bhPx*0.35);
      ctx.font=`${Math.floor(bhPx*0.3)}px Gonta`;
      ctx.fillText(pl1.skills[i].cost, x0+bwPx/2, y0m+bhPx*0.75);
    }
